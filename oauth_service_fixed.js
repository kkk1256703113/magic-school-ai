...
class OAuthService {
    ...
    async findOrCreateUser({ email, name, provider, providerId, avatarUrl }) {
        try {
            ...
            // 为OAuth用户生成特殊的密码哈希（表示使用OAuth登录）
            const oauthPasswordHash = await bcrypt.hash(`oauth_${provider}_${providerId}`, 10);
            
            const insertResult = await this.pool.query(
                `INSERT INTO users (
                    email, username, password_hash, oauth_provider, oauth_id, avatar_url, 
                    plan_type, api_calls_today, created_at, last_login
                ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())
                RETURNING *`,
                [email, name, oauthPasswordHash, provider, providerId, avatarUrl, 'free', 0]
            );
